...
Core.prototype.reverselookup = function(relativePath) {
  ...
  var extensions = [".html"].concat(this.extensions);

  var file = findFile(fullpath, extensions);

  if(file) {
    return file;
  } else {
    return this.resolve(fullpath, basename);
  }
};
...
function findFile (filename, extensions) {
  if (isFile(filename)) {
    return filename;
  }
    
  for (var i = 0; i < extensions.length; i++) {
    var file = filename + extensions[i];
    if (isFile(file)) {
      return file;
    }
  }
