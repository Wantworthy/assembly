...
var Manager = exports.Manager = function (options) {  
  ...
  this.on("compile", this.compileAndWrite);
...
Manager.prototype.callIfModified = function(src, cb){
  ...
  async.parallel({
      ...
      destStats: async.apply(fs.stat, this.destFile(src))
    },
...
Manager.prototype.write = function(srcFile, data, cb) {
  ...
  var self = this;
  var outputFile = this.destFile(srcFile);
...
Manager.prototype.remove = function(file) {
  ...
  var delFile = this.destFile(file);
...
Manager.prototype.destFile = function(filePath) {
  ...
  return path.join(path.dirname(filePath), newFileName).replace(this.src, this.dest);
};