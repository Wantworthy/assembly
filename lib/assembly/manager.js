...
Manager.prototype.server = function() {
  ...
  return function(req, res, next) {
    ...
    var fullpath = path.join(self.src, pathname);  
    var sourceFile = self.core.resolve(fullpath, path.basename(fullpath, path.extname(fullpath)));
    if(sourceFile) {
      self.compileAndWrite(sourceFile, function(err, data) {
        return next();
      })
    } else {
      return next();
